...
public class GenerarPregunta : MonoBehaviour {
	...
	void Start () {
        ...
        switch(materia){
            case 0:
                textFile = Resources.Load<TextAsset>("Ciencias");
                preguntas= textFile.text.Split('\n');
                textFileR = Resources.Load<TextAsset>("RCiencias");
                respuestas= textFileR.text.Split('\n');
                if(preguntas!=null){
                    funciona=true;
                }
                break;
            case 1:
                textFile = Resources.Load<TextAsset>("Fisica");
                preguntas= textFile.text.Split('\n');
                textFileR = Resources.Load<TextAsset>("RFisica");
                respuestas= textFileR.text.Split('\n');
                if(preguntas!=null){
                    funciona=true;
                }
                break;
            case 2:
                textFile = Resources.Load<TextAsset>("Geografia");
                preguntas= textFile.text.Split('\n');
                textFileR = Resources.Load<TextAsset>("RGeografia");
                respuestas= textFileR.text.Split('\n');
                if(preguntas!=null){
                    funciona=true;
                }
                break;
            case 3:
                textFile = Resources.Load<TextAsset>("Historia");
                preguntas= textFile.text.Split('\n');
                textFileR = Resources.Load<TextAsset>("RHistoria");
                respuestas= textFileR.text.Split('\n');
                if(preguntas!=null){
                    funciona=true;
                }
                break;
            case 4:
                textFile = Resources.Load<TextAsset>("Matematicas");
                preguntas= textFile.text.Split('\n');
                textFileR = Resources.Load<TextAsset>("RMatematicas");
                respuestas= textFileR.text.Split('\n');
                if(preguntas!=null){
                    funciona=true;
                }
